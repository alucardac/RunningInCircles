...
public class MapEditor : MonoBehaviour
{
	...
	private int mapID;
	[SerializeField]
	private BoxCollider2D backgroundPlane;
	[SerializeField]
	private Transform backgroundGeometry;
	[SerializeField]
	...
	private MapTile CreateAtPosition(int xPos, int yPos, Transform targetRoot, Quaternion targetRotation)
	{
		...
		if (newMapTile == null)
		{
			newTileGo.AddComponent<MapTile>();
		}
		newTileTr.position = new Vector3(xPos, yPos, newMapTile.zDepth);
	...
	private void AssignTeamPropertiesToObject(Transform target, PlayerTeam team)
	{
		...
		{
			CharacterBase newCharacter = target.GetComponent<CharacterBase>();
			if (newCharacter != null)
			{
				...
				newCharacter.spawnRotation = newCharacter.transform.rotation;
			}
		}
	}
